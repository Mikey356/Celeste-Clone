...
void play_sound(char* soundName, SoundOptions options = 0)
{
    ...
    if(!(options & SOUND_OPTION_START) && !(options & SOUND_OPTION_FADE_IN) && !(options & SOUND_OPTION_FADE_OUT))
    {
    ...
    if(wavFile)
    {
        ...
        {
            SM_ASSERT(0, "Exhausted Sounds Buffer!\nCapacity:\t%d\nBytes Used:\t%d\nSound Path:\t%s\nSound Size:\t%d", SOUNDS_BUFFER_SIZE, soundState->bytesUsed, sound.file, wavFile->header.dataChunkSize);
            return;
        }
